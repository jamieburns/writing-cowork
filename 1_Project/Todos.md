...
| c3f80b6e | **The post-commit hook is silent when `drift_check.py` cannot run.** `drift_check.py:60-64` writes "ERROR: PyYAML not installed" to **stderr** and exits 2; `templates/hooks/post-commit:38` runs it with `2>/dev/null` and greps stdout for 'session hygiene'. Result: empty output, hook prints nothing, commit looks clean | v0.1.17-release | pm | planned | 2026-07-30 | Found 2026-07-30 on the first real host run. Host python3 is `/opt/homebrew/bin/python3` and has **no PyYAML**, so the hook is inert on Jamie's own machine right now. Fifth instance of the silent-no-op class — and it is in the release-blocking component itself |
...
| 7e4b1a93 | `drift_check.py` hardcodes consumer-layout paths: `config.vault / "process/active/todos.md"` (lines 544, 554) and `"process/active/roadmap.md"` (line 587). Not config keys — no vault on a different layout can point the todos/roadmap checks anywhere | v0.1.18 | pm | planned | 2026-07-30 | Found 2026-07-30 scoping `0d92f4a7`. Distinct from `a4e1c7b2` (columns) and `c6d05a91` (inert check) — same *class*: the checker returns "clean" instead of "input not found". **2026-07-31:** drift_check 0.4.0 preflight now *reports* both paths as unreachable, so the silence is gone; the hardcoding itself is still open |
| d51c9a27 | Correct the "read-only inspection is fine anywhere" clause in the router invariant — `git status` creates `.git/index.lock` and orphans it from a mounted view. Source is the MANAGED block, so fix `0_Product/skills/pm-install-router/SKILL.md` and `templates/CLAUDE.md`, not `CLAUDE.md` directly | v0.1.18 | pm | planned | 2026-07-30 | Hand-editing `CLAUDE.md` would be silently reverted on the next sync. Evidence in `1_Project/Memory/feedback_git_status_creates_index_lock.md` |
